...
public class ReceiveScreen : SimpleScreen
{
    ...
    private void Start()
    {
        airdrop_btn.onClick.AddListener(async () => {
            await WalletService.Instance.Wallet.RequestAirdrop();
        });
    ...
    {
        var isDevnet = WalletService.Instance.Wallet?.RpcCluster == RpcCluster.DevNet;
    ...
    public override void ShowScreen(object data = null)
    {
        ...
        GenerateQR();
        publicKey_txt.text = WalletService.Instance.Wallet.Account.PublicKey;
    }

    private void CheckAndToggleAirdrop()
    {
        airdrop_btn.gameObject.SetActive(!WalletService.Instance.Wallet.ActiveRpcClient.ToString().Contains("api.mainnet"));
    }

    private void GenerateQR()
    {
        Texture2D tex = QRGenerator.GenerateQRTexture(WalletService.Instance.Wallet.Account.PublicKey, 256, 256);
        qrCode_img.texture = tex;
    }

    public void CopyPublicKeyToClipboard()
    {
        Clipboard.Copy(WalletService.Instance.Wallet.Account.PublicKey.ToString());
