...
namespace Solana.Unity.SDK.Example
{
    public class WalletScreen : SimpleScreen
    {
        ...
        public void Start()
        {
            ...
            WalletService.WsRpc.SubscribeAccountInfo(
                WalletService.Instance.Wallet.Account.PublicKey,
                (_, accountInfo) =>
                {
        ...
        {
            var hasPrivateKey = !string.IsNullOrEmpty(WalletService.Instance.Wallet?.Account.PrivateKey);
            savePrivateKeyBtn.gameObject.SetActive(hasPrivateKey);
            var hasMnemonics = !string.IsNullOrEmpty(WalletService.Instance.Wallet?.Mnemonic?.ToString());
        ...
        private void SavePrivateKeyOnClick()
        {
            ...
            if (string.IsNullOrEmpty(WalletService.Instance.Wallet.Account.PrivateKey?.ToString())) return;
            Clipboard.Copy(WalletService.Instance.Wallet.Account.PrivateKey.ToString());
        ...
        {
            if (!gameObject.activeSelf) return;
            if (string.IsNullOrEmpty(WalletService.Instance.Wallet.Mnemonic?.ToString())) return;
            Clipboard.Copy(WalletService.Instance.Wallet.Mnemonic.ToString());
        ...
        {
            if (WalletService.Instance.Wallet.Account is null) return;
            var sol = await WalletService.Base.GetBalance(Commitment.Confirmed);
        ...
        {
            var tokens = await WalletService.Base.GetTokenAccounts(Commitment.Confirmed);
                ...
                foreach (var item in tokenAccounts)
                {
                    ...
                    if (_instantiatedTokens.All(t => t.TokenAccount.Account.Data.Parsed.Info.Mint != item.Account.Data.Parsed.Info.Mint))
                    {
                        ...
                        Nft.Nft.TryGetNftData(item.Account.Data.Parsed.Info.Mint,
                            WalletService.Instance.Wallet.ActiveRpcClient).AsUniTask().ContinueWith(nft =>
                        {
